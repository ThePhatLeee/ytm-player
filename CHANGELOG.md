...
- **Spotify import's scraper fallback was broken** — `spotifyscraper` (an optional dependency) renamed its `get_playlist_info()` method to `get_playlist()` and changed its return type from a dict to a dataclass at its 3.0.0 release; the unpinned `>=2.0.0` dependency bound let pip install a version whose API no longer matched what this code expected, so importing without Spotify API credentials configured raised `AttributeError`. Updated to the new API and raised the dependency floor to `>=3.0.0`. While fixing this, also dropped the scraper's own ~100-track cap — it now fetches full playlists the same as the credentialed path already did.
- **Missing album art on some radio tracks** — `normalize_tracks()` only read the `thumbnails` (plural) key, but tracks from `get_watch_playlist(radio=True)` carry their thumbnail list under the singular `thumbnail` key instead. Radio and mix tracks now show album art consistently.
- **Seek keys `>` `<` `^` did nothing on a fresh install** (#143) — Textual reports punctuation keys by name (`greater_than_sign`, `circumflex_accent`, …) and the key handler only translated five of them, so the default seek bindings never matched. Any printable key now matches its literal character, so every punctuation binding in `keymap.toml` works as written.
- **`ytm setup --manual` accepts Chrome's headers as pasted** — Chrome DevTools annotates `x-client-data` with a pretty-printed protobuf under a `Decoded:` line. That block spans an odd number of lines, which shifted the alternating name/value pairing of every header below it, so `x-goog-authuser` was read as a value and setup failed with "The following entries are missing in your headers: x-goog-authuser" — on a paste that contained it. The block is now stripped before pairing. Affected any Chrome user following the documented setup steps; Firefox's `Name: Value` copy format was never affected.
- **Expired browser sessions renew automatically** — on an authentication failure, ytm shows refresh progress, reuses browser cookies efficiently, preserves the selected Google account, rebuilds the client, and retries the operation once instead of immediately asking the user to run `ytm setup`.
- **Automatic renewal can no longer switch you to another Google account** — the renewal above picked the first browser account that still validated when the saved one didn't, and then retried the pending action (a like, a playlist edit, history sync) as that account. `ytm setup` now records which account the session belongs to (`account.json`, bound to the exact `auth.json` it describes) and renewal only accepts a session with the same YouTube channel ID, in the saved browser slot or in exactly one other slot after a browser re-order. No recorded identity, a mismatch, or an ambiguous match refuses the renewal and the action is not retried; the retry is also bound to the account of the client the action failed on, so an `ytm setup` for another account landing while it was pending never replays it as that account. Sessions set up before this version keep working until they expire; when renewal is first needed, run `ytm setup` once.
- **Sorted playlist loads no longer skip session renewal** — the Library page loads playlists sorted by recently added, and that path returned an empty playlist on an expired session instead of renewing like every other request. It now renews outside the sort window and retries the sorted load once on the renewed session, with the same sort, limit and timeout.
- **A burst of requests on an expired session no longer repeats a failed renewal** — after the first renewal attempt failed, every request still waiting re-ran the browser cookie extraction and account probes. A failed renewal now backs off for 60 seconds: requests in that window fail with the session-expired message without touching the browser, and the next attempt after it picks up a browser you have signed back into.
- **Playback silently did nothing after a network or VPN change** (#135) — stream URLs are bound to the IP that fetched them, and a stream that failed to open was never handled: no retry, no skip, no message. A failed stream now drops the cached URLs and the yt-dlp instance and retries that track once; if the retry fails too, the track is skipped with a message and counts towards the existing consecutive-failure limit. Errors for a play that a newer play, a stop, or a replay has already superseded are ignored, and a load failure inside mpv is reported once instead of being swallowed.
- **Switching themes lost part of the base theme** — the rebuild that makes cursor and selection colours follow `theme.toml` (#123) copied only the colours, so a theme's terminal-default (`ansi`) mode, panel, boost, luminosity spread and text alpha fell back to defaults on every switch, with or without a `theme.toml`: `ansi-dark` stopped using the terminal's own colours and `nord` lost its panel colour. The rebuild now keeps every other field of the base theme.
- **Album-art placeholder glyph unreadable on light themes** — the readable-colour pick from #123 assumed the theme foreground is the light colour and the background the dark one; on a light theme that is reversed, so a pale accent got white text. The glyph now uses whichever of the two has the higher WCAG contrast ratio against the accent, and keeps the foreground when the colours can't be compared (terminal-dependent colours).
- **Cursor/selection highlights and the album-art placeholder now actually follow custom themes** — a `theme.toml` with a light/pastel `primary` used to leave the track-table cursor and sidebar selection stuck on the stock theme's red/dark-maroon, and made the album-art placeholder's ♫ glyph nearly invisible. The cursor highlight now re-registers the active `Theme` with the resolved colors so Textual's own derived design tokens (`block-cursor-background`, etc.) follow `theme.toml` too, not just the app-specific CSS variables; the placeholder glyph now picks light-or-dark text by contrast against its background instead of always using `theme.foreground`. (#122)
- **Zen Browser is now detected by `ytm setup`** (#119) — Zen keeps Firefox-format profiles in its own directory, which yt-dlp does not search. ytm-player now points the Firefox extractor at Zen's profile root (`~/.zen`, `~/.config/zen`, the Flatpak dirs, `~/Library/Application Support/zen`, `%APPDATA%\zen`), and `ytm setup --browser zen` works.
- **Playlist sidebar stopped after ~74 playlists** (#134) — the sidebar and the add-to-playlist picker asked the library for 50 playlists and got whole pages back, so larger libraries were cut off. Both now ask for all of them.
- **Artist pages showed only 10 albums and 10 singles** (#138) — the artist call returns a preview of each section and the app never asked for the rest. The page now fetches the full albums and singles lists (Metallica: 27 and 24 instead of 10 and 10).
- **Streaming can use your `ytm setup` session** — opt in with `[yt_dlp] use_session_cookies = true` and the stream resolver loads the youtube.com/google.com cookies that `ytm setup` (browser or manual header paste) and the automatic session refresh write to `stream_cookies.txt`, so tracks that need your account (Premium-only, age-restricted) resolve as you. This mode relies on yt-dlp's JavaScript challenge solver (a local `yt-dlp-ejs` install or `remote_components = "ejs:github"`, plus a supported JS runtime); without a working solver authenticated playback may fail — see [Configuration](docs/configuration.md#yt_dlp). Off by default: streaming stays anonymous and needs nothing extra. The resolver never decrypts browser cookies on its own, a cookie file rewritten mid-session by an auto-refresh is picked up on the next resolve, and yt-dlp never writes the file back. The format fallback now requires a stream with an audio track. Thanks @wgordon17 (#136).
